...
\begin{abstract}[zh]
    ...
    针对上述问题，本文提出了一种面向混有模型数据的云存储负载的具有细粒度冗余识别能力的数据缩减系统——FHRD (Fine-grained Hybrid Redundancy Deduplication)。该系统在传统数据缩减流水线的基础上，引入了子块粒度的冗余识别与去重，以及模型数据的鉴别、分离与编码压缩能力。
    ...
    本文实现了 FHRD 原型，并进行了全面的实验评估。实验结果表明，在包含模型数据与传统数据的混合负载下，FHRD 相比 Destor 等传统“变长分块+块级去重+通用压缩”方案，在保持良好吞吐性能的同时，平均数据缩减率提升了 21.7\%，对比 Zipnn、Burst 等针对优化方案也有不同程度的提升。
