...
\sjtusetup{
  ...
  info = {%
    ...
    % 标题
    %
    zh / title           = {细粒度冗余识别的重复数据删除系统研究},
    en / title           = {Research on Deduplication System with Fine-Grained Redundancy Identification},
    %
    % 标题页标题
    %   可使用“\\”命令手动控制换行
    %
    ...
    % 关键词
    %
    zh / keywords        = {存储, 重复数据删除, 细粒度, AI模型, 混合负载, 分治},
    en / keywords        = {storage, deduplication, fine-grained, AI model, mixed workload, divide-and-conquer},
    %
    % 姓名
    %
    zh / author          = {刘\quad{}茁},
