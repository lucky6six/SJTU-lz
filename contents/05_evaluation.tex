...
为评估优化技术的贡献，本文设计了消融对比（FHRD-1、FHRD-2）分别去除或替换关键子模块以量化其影响。

另外，为了与已有细粒度或专用方法做对比，在非模型数据去重方面，本研究实现了 BURST 及其与变长分块结合的变体（BURST+），
并将 ODESS（全局字节粒度增量压缩）纳入对照以代表理想化的增量压缩上界；需要指出的是，ODESS 在计算与内存开销上远高于
在线云存储可接受的范围，因此更适合作为效果上的衡量，而非直接可部署的在线方案。

针对模型数据的专用编码压缩方法（如 ZIPNN、DIFFNN），为公平比较本研究也实现了若干组合变体（ZIPNN+、ZIPNN++），以
...
\begin{table}[!hpt]
    ...
    \begin{tabular}{@{} >{\raggedright\arraybackslash}p{0.2\linewidth}
        ...
        fhrd(本文系统) & 本文提出的面向混合负载的细粒度冗余识别的去重系统，设计实现参照第 3、4 章 \\\midrule
        fhrd-1 & FHRD 消融实验版本 1，模型处理模块去除基于熵值分析结论的字节分组压缩，改为全部压缩 \\\midrule
        fhrd-2 & FHRD 消融实验版本 2，非模型处理模块去除指数取整的双向子块定长分块方法，仅均分子块 \\\midrule
        burst(MSST24) & 面向块内突发修改的去重系统，采用头尾指纹技术去除大块边缘冗余 \\\midrule
        burst+ & BURST + 变长分块技术 \\\midrule
        odess(TOS23) & 全局匹配的字节粒度增量压缩去重系统\\\midrule
        zipnn(CLOUD2025) & 针对模型文件的编码去重系统\\\midrule
...
本文在 5.1.2 节中所述的各个实验对象上分别运行存储任务，直至去重、压缩与持久化流程完全结束。通过精确记录存储前后数据量的变化和任务总执行时间，
本文计算出各方案的任务数据缩减率和吞吐量，旨在深入对比和分析在当前云存储环境下，FHRD 相较于其他现有工作的实际效果与性能差异。
...
如图~\ref{fig:5.2.1} 所示为各类混合负载下不同方案的数据缩减率对比结果。从图中可以观察到几个关键现象：
\begin{itemize}
...
可以观察到一个宏观趋势：去重效果越精细，性能开销越大。这一方面源于更细粒度的冗余处理（如块内去重、模型编码）本身带来的额外计算开销；
另一方面，这也与通用压缩算法（如ZSTD）的特性相关——当输入数据的冗余度更高（即可压缩性更强）时，压缩算法需要消耗更多时间来查找和编码重复模式，从而导致处理速度下降。

综合 5.2 节的实验结果，可以得出以下结论：单纯基于定长分块的优化方案（如 ZIPNN, BURST）已无法满足现代复杂负载的需求，
因此在后续的实验中，将重点与它们的变长分块优化版本进行比较。同时，尽管 ODESS 在去重效果上表现优异，但其过高的性能开销使其不具备在线应用的可行性，
后续仅将其作为衡量去重效果的理想上界进行参考和讨论。
...
为此，本文设计了两种 FHRD 的消融版本：FHRD-1 和 FHRD-2（具体定义见表~\ref{tab:tested_schemes}），旨在量化评估每个模块在不同场景下的实际效果与性能影响。
...
为进一步剥离块级去重和通用压缩等因素的干扰，本研究设计了更精细的实验，直接衡量不同方法在“相似块”检出和冗余去除上的核心能力。
...
更高的相似块检出率是实现更优去重效果的直接前提。如图~\ref{fig:5.3.3.4} 所示，在对已检出的相似块进行去重时，本研究观察到一个有趣的现象：FHRD 的“相似块冗余数据缩减率”反而低于其他方法。这并非说明 FHRD 的去重能力更弱，
...
综合本节所有实验结果，可以得出结论：“指数取整的双向子块定长分块方法”通过其创新的分块策略，能够以可接受的微小性能开销，换取相似块检出率的巨大提升。这种能力使得 FHRD 能够发现并消除更多传统方法无法触及的块内冗余，最终转化为显著的整体去重效果提升，充分证明了该方法在处理非模型数据冗余方面的实用性与先进性。
...
因此，本文将“模型数据块”操作性地定义为：能够通过 4.5 节所述的编码压缩方法实现 10\% 以上空间节省的数据块。这个标准将作为类型鉴别准确性的基准。

实验中，对比了三种不同的鉴别方法在多个典型模型数据集上的表现：
\begin{enumerate}
...
\end{enumerate}
本研究采用正确率、误报率和漏检率这三个关键指标来全面衡量它们的准确性。
...
\begin{figure}[htbp]
    ...
    \bicaption{类型鉴别误报率对比}{Comparison of false positive rate}
    \label{fig:5.3.1.2}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \bicaption{类型鉴别漏检率对比}{Comparison of false negative rate}
    \label{fig:5.3.1.3}
\end{figure}

为深入探究正确率差异的根源，本文进一步分析了各方法的误报率和漏检率，如图~\ref{fig:5.3.1.2} 和图~\ref{fig:5.3.1.3} 所示。文件级方法的问题主要体现在两个方面：
\begin{itemize}
...
最后，本文评估了这几种方法的时间开销。文件级方法的开销与文件数量相关，与数据总量关系不大，其耗时极低，几乎可以忽略不计。
因此，本实验重点对比了字节级和比特级方法的吞吐量，如图~\ref{fig:5.3.1.4} 所示。实验结果清晰地表明，字节级方法的吞吐量显著高于比特级方法，
...
在验证了类型鉴别的有效性后，本节将深入评估 FHRD 针对模型数据的优化——基于熵值分析结论的字节分组压缩方法。本文将通过对比 FHRD、其消融版本 FHRD-1 以及专为模型数据设计的 ZIPNN++，来分析该方法在去重效果和性能开销上的综合表现。

如图~\ref{fig:5.3.2} 所示，比较了这几种方案在不同模型数据集上的数据缩减率。实验结果揭示了以下几点：
\begin{itemize}
...
此外，通过对比 FHRD 与其消融版本 FHRD-1（该版本对所有字节组进行压缩，而非选择性压缩），发现 FHRD 的数据缩减率并未下降，反而在多个数据集上略有提升。
...
为了进一步验证该方法在节省计算资源方面的优势，本文测量了压缩线程的吞吐量，如图~\ref{fig:5.3.2.2} 所示。实验数据显示，ZIPNN++ 和 FHRD-1 的压缩吞吐量均低于基准的 BASE 方案。
...
FHRD 及其他方案去重效果的影响。实验负载选用 \texttt{dataset} 和 \texttt{checkpoints}，因为这两个人工生成的数据集能更精确地控制模型数据的占比和版本间的差异。
...
首先，本文考察云存储负载的第一个变化趋势：工作负载中模型数据的比例日益增加。
如图~\ref{fig:5.4.1} 所示，绘制了各系统数据缩减率随模型数据比例变化的曲线。
\begin{itemize}
...
接下来，本文探讨云存储负载的第二个变化趋势：为了适应海量数据的存储和管理，数据块的平均大小越来越大。
如图~\ref{fig:5.4.2} 所示，展示了各系统数据缩减率随块大小变化的趋势。
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{关键技术消融分析}：通过细致的消融实验，逐一剖析了 FHRD 内部各项关键技术的实际贡献：
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{负载影响分析}：最后，通过对混合负载比例和块大小这两个关键参数进行对比实验，发现 FHRD 不仅在各种数据混合比例下都能保持领先，而且能够有效抵抗因块尺寸增大而导致的数据缩减效率衰减问题。
\end{itemize}
