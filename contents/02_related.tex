...
其中去重（通常为块级去重）由数据分块、指纹计算、匹配去重三个关键步骤构成，与数据压缩共同组成一个完整的数据缩减工作流。
...
如图~\ref{fig:variable_length}所示，内容定义分块（Content-Defined Chunking, CDC），又称变长分块，是为解决固定分块的边界偏移问题而设计的关键技术。其核心思想是根据数据内容自身的特征来动态确定分块边界，从而使得分块结果对数据的插入和删除操作具有良好的鲁棒性~\cite{muthitacharoen2001low}。
...
\section{本章小结}
本章首先深入剖析了传统去重系统的工作原理，详细探讨了数据切块（固定大小与内容定义）、哈希指纹与索引、以及数据压缩等关键技术环节，并指出了其在面对新型数据（如 AI 模型）时暴露出的局限性。
