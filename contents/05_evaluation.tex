...
值分析结论的字节分组压缩方法和指数取整的双向子块定长分块方法等关键技术在不同类型负载下的实际效果，验证了各优化技术的优越性，以及系统在各种数据集上的适应情况。
...
\begin{table}[!hpt]
    ...
    \begin{tabular}{@{} >{\raggedright\arraybackslash}p{0.15\linewidth}
        ...
        \toprule
        对照组 & 方案说明 \\ \midrule
        BASE & 基础对照组，在开源系统 Destor 的基础集成 ZSTD 压缩算法，代表目前工业界主流的变长分块块级去重加通用压缩的传统方案。 \\\midrule
...
此外，本文还设计了人工数据集 dataset和训练数据集 checkpoints，以便在定量实验中更精细地控制模型数据占比。
...
本节将通过一系列消融实验和对比分析，对第四章提出的各项优化——包括类型鉴别模块、基于熵值分析的字节分组压缩以及指数取整的双向子块定长分块方法——进行逐一剖析。
...
本节将聚焦于 FHRD 针对非模型数据的优化——“指数取整的双向子块定长分块方法”，旨在阐明其在处理块内冗余方面的技术优势。
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item 在 \texttt{backup}、\texttt{code} 等数据集中，不同方法之间的数据缩减率差距被显著放大。在这些场景下，FHRD 的优势尤为突出，表明其“指数取整的双向子块定长分块”方法在处理包含大量版本迭代、代码复制、数据块移位等产生的块内冗余时，表现极为出色。
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{指数取整双向子块分块}：此方法被证明能够以极小的性能代价，换取对块内冗余（尤其是在非模型数据中）识别能力的巨大提升，是 FHRD 实现高数据缩减率的关键。
    \end{itemize}
    \item \textbf{负载影响分析}：最后，通过对混合负载比例和块大小这两个关键参数进行扫描实验，我们发现 FHRD 不仅在各种数据混合比例下都能保持领先，而且能够有效抵抗因块尺寸增大而导致的去重效率衰减问题。
\end{itemize}
综上所述，本章的实验结果充分验证了 FHRD 作为一个整体设计的先进性，以及其各项核心优化技术的优越性。实验数据有力地证明，本文提出的方法能够有效应对当前云存储环境中数据日益混合化和块尺寸增大的挑战，实现了在复杂负载下的高效去重。
